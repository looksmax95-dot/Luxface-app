...
/* ============================================================
   13A. PROFESSIONAL HARM — FRONT POINT ENGINE
   ============================================================ */

function professionalTier(value, tiers) {
  if (!Number.isFinite(value) || !Array.isArray(tiers)) {
    return null;
  }
...
function findHarmMetric(id) {
  for (var i = 0; i < HARM_METRICS.length; i++) {
    if (HARM_METRICS[i].id === id) {
      return HARM_METRICS[i];
    }
  }
...
function addHarmAssignment(assignments, id, value, tier) {
  var metric = findHarmMetric(id);

  if (!metric || !Number.isFinite(value) || tier == null) {
    return;
  }
...
export function computeProfessionalHarm(P, bizy, fh) {
  ...
  /*
     ...
     Здесь используются ТОЛЬКО признаки, которые реально
     можно получить из текущего frontal point set.

     Никаких придуманных side-profile значений.
  */
  ...
  */
  ...
  if (jawWidth >= 87.5 && jawWidth <= 91.5) {
    jawTier = 0;
  } else if (
    jawWidth >= 85 &&
    jawWidth <= 94
  ...
  var rightBrow =
    safeDivide(
      dist(P.eyeRc, P.browRp),
      Math.max(1, dist(P.eyeRo, P.eyeRi)),
      0
    );

  var leftBrow =
    safeDivide(
      dist(P.eyeLc, P.browLp),
      Math.max(1, dist(P.eyeLo, P.eyeLi)),
      0
    );

  var browSetness =
    (rightBrow + leftBrow) / 2;


  /*
     Lower value = closer brows.

     Current implementation uses a normalized
     eye-width ratio because that is what the
     available point geometry can actually provide.
  */
  ...
  if (browSetness <= 0.55) {
    browTier = 0;
  } else if (browSetness <= 0.70) {
    browTier = 1;
  } else if (browSetness <= 0.85) {
    browTier = 2;
  } else if (browSetness <= 1.00) {
    browTier = 3;
  } else if (browSetness <= 1.20) {
    browTier = 4;
  } else {
    browTier = 5;
  }
  ...
  */
  ...
  var thirdsError =
    Math.abs(upper - 31) +
    Math.abs(middle - 32.4) +
    Math.abs(lower - 35.45);
  ...
  if (thirdsError <= 3) {
    thirdsTier = 0;
  } else if (thirdsError <= 5) {
    thirdsTier = 1;
  } else if (thirdsError <= 8) {
    thirdsTier = 2;
  } else if (thirdsError <= 12) {
    thirdsTier = 3;
  } else if (thirdsError <= 18) {
    thirdsTier = 4;
  } else {
    thirdsTier = 5;
  }
  ...
  if (lower >= 33.9 && lower <= 37.0) {
    lowerTier = 0;
  } else if (lower >= 32 && lower <= 39) {
    lowerTier = 1;
  } else if (lower >= 30 && lower <= 41) {
    lowerTier = 2;
  } else if (lower >= 28 && lower <= 43) {
    lowerTier = 3;
  } else if (lower >= 25 && lower <= 46) {
    lowerTier = 4;
  } else {
    lowerTier = 5;
  }
  ...
  if (fwhr >= 1.95 && fwhr <= 2.05) {
    fwhrTier = 0;
  } else if (fwhr >= 1.90 && fwhr <= 2.10) {
    fwhrTier = 1;
  } else if (fwhr >= 1.85 && fwhr <= 2.15) {
    fwhrTier = 2;
  } else if (fwhr >= 1.80 && fwhr <= 2.20) {
    fwhrTier = 3;
  } else if (fwhr >= 1.70 && fwhr <= 2.30) {
    fwhrTier = 4;
  } else {
    fwhrTier = 5;
  }
  ...
  if (eyeAspect >= 3.0 && eyeAspect <= 3.7) {
    eyeAspectTier = 0;
  } else if (eyeAspect >= 2.8 && eyeAspect <= 3.9) {
    eyeAspectTier = 1;
  } else if (eyeAspect >= 2.6 && eyeAspect <= 4.1) {
    eyeAspectTier = 2;
  } else if (eyeAspect >= 2.4 && eyeAspect <= 4.3) {
    eyeAspectTier = 3;
  } else if (eyeAspect >= 2.2 && eyeAspect <= 4.5) {
    eyeAspectTier = 4;
  } else {
    eyeAspectTier = 5;
  }
  ...
  if (mouthNose >= 1.45 && mouthNose <= 1.75) {
    mouthNoseTier = 0;
  } else if (mouthNose >= 1.35 && mouthNose <= 1.85) {
    mouthNoseTier = 1;
  } else if (mouthNose >= 1.25 && mouthNose <= 1.95) {
    mouthNoseTier = 2;
  } else if (mouthNose >= 1.15 && mouthNose <= 2.05) {
    mouthNoseTier = 3;
  } else if (mouthNose >= 1.00 && mouthNose <= 2.20) {
    mouthNoseTier = 4;
  } else {
    mouthNoseTier = 5;
  }
  ...
  if (midface >= 0.95 && midface <= 1.01) {
    midfaceTier = 0;
  } else if (midface >= 0.92 && midface <= 1.04) {
    midfaceTier = 1;
  } else if (midface >= 0.89 && midface <= 1.07) {
    midfaceTier = 2;
  } else if (midface >= 0.86 && midface <= 1.10) {
    midfaceTier = 3;
  } else if (midface >= 0.82 && midface <= 1.14) {
    midfaceTier = 4;
  } else {
    midfaceTier = 5;
  }
  ...
  if (jfa >= 84.5 && jfa <= 95) {
    jfaTier = 0;
  } else if (jfa >= 82 && jfa <= 98) {
    jfaTier = 1;
  } else if (jfa >= 79 && jfa <= 101) {
    jfaTier = 2;
  } else if (jfa >= 76 && jfa <= 104) {
    jfaTier = 3;
  } else if (jfa >= 72 && jfa <= 108) {
    jfaTier = 4;
  } else {
    jfaTier = 5;
  }
  ...
  if (bitemporal >= 84 && bitemporal <= 95) {
    temporalTier = 0;
  } else if (bitemporal >= 80 && bitemporal <= 99) {
    temporalTier = 1;
  } else if (bitemporal >= 76 && bitemporal <= 103) {
    temporalTier = 2;
  } else if (bitemporal >= 72 && bitemporal <= 107) {
    temporalTier = 3;
  } else if (bitemporal >= 68 && bitemporal <= 112) {
    temporalTier = 4;
  } else {
    temporalTier = 5;
  }
